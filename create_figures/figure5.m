...
%% Preprocess and format data before displaying
   % visualization parameters
      ...
      disp.colors = [0 0 0; 5 113 176]./255; 


   % load bootstrap samples for exogenous attention fits
      boot = combine_bootstrap_iterations('model_variant','main_model','attn_type','exo');
   ...
   % compute attentional gain on the target 
      ...
      gain_on_targ.ecc              = ecc;
      for b = 1:size(boot.fit_err,1) % loop through bootstrap samples
         for e = 1:size(boot.fit_err,2) % ...and experiments
            for ec = 1:numel(ecc) % and compute the gain on the target frequency at each eccentricity
               ...
               gaintarg(ec,e,b)  = make_cosine_fun(log2(targfreq),attnfreq,attnbw*2,1);
            end
         end
      end
      gain_on_targ.ecc              = ecc; 
      % fine textures
         ...
         gain_on_targ.fine.center   = median(gain_on_targ.fine.all(:,:),2)';
      ...
      % coarse textures
         ...
         gain_on_targ.coarse.center = median(gain_on_targ.coarse.all(:,:),2)';
   ...
   % generate matrix images showing SF selectivity of attention
      % load texture stimulus for simulation
         stim        = load(sprintf('../data/behavior/%s.mat','yc98_exp1'));         
         modelp      = stim.params;
         stim        = stim.data.stim;
      ...
      % run the simulation
         [~, tmp] = imAmodel(stim.targ,'ecc',ecc,'stimdrive',stimdrive,'supdrive',supdrive,'attn',attn,'use_attn',1);
...
figure('name','Figure 5');
   ...
   % Scatter plot
      ...
      % pretty it up
         ...
         legend(leg,{'neutral' 'peripheral'},'fontname','arial','fontsize',6,'location','northwest');
